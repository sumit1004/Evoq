...
import { mutationRateLimiter, registrationRateLimiter } from '../middleware/securityMiddleware.js';
...
tournamentRouter.post('/', mutationRateLimiter, authenticateRequest, requireRoles('ORGANIZER', 'ADMIN'), uploadPaymentQr, validateRequest({ body: validateTournamentCreate }), createTournamentController);
...
tournamentRouter.patch('/:tournamentId', mutationRateLimiter, authenticateRequest, requireRoles('PLAYER', 'ORGANIZER', 'ADMIN'), uploadPaymentQr, validateRequest({ params: validateTournamentId, body: validateTournamentPatch }), updateTournamentController);
tournamentRouter.delete('/:tournamentId', mutationRateLimiter, authenticateRequest, requireRoles('ORGANIZER', 'ADMIN'), validateRequest({ params: validateTournamentId }), deleteTournamentController);
...
tournamentRouter.put('/:tournamentId/scoring-config', mutationRateLimiter, authenticateRequest, requireRoles('ORGANIZER', 'ADMIN'), validateRequest({ params: validateTournamentId }), updateScoringConfigController);
...
tournamentRouter.post('/:tournamentId/registrations', registrationRateLimiter, authenticateRequest, requireRoles('PLAYER'), uploadPaymentEvidence, validateRequest({ params: validateTournamentId, body: validateRegistrationCreate }), createRegistrationController);
...
registrationRouter.post('/bulk-verify', mutationRateLimiter, authenticateRequest, requireRoles('PLAYER', 'ORGANIZER', 'ADMIN'), bulkVerifyController);
registrationRouter.post('/bulk-reject', mutationRateLimiter, authenticateRequest, requireRoles('PLAYER', 'ORGANIZER', 'ADMIN'), bulkRejectController);
...
registrationRouter.patch('/:registrationId', mutationRateLimiter, authenticateRequest, requireRoles('PLAYER', 'ORGANIZER', 'ADMIN'), validateRequest({ params: validateRegistrationId, body: validateRegistrationReview }), reviewRegistrationController);
