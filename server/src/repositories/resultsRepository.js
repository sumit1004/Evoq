...
export async function listMatchResults(matchId) { const [rows] = await pool.query(`SELECT mr.id, mr.match_id, mr.team_id, t.name AS team_name, mr.points, mr.kills, mr.placement, mr.kill_points, mr.position_points, mr.result_text, mr.media_path, mr.uploaded_by, mr.created_at, mr.updated_at FROM match_results mr JOIN teams t ON t.id = mr.team_id WHERE mr.match_id = ? ORDER BY mr.points DESC, mr.kills DESC, mr.placement IS NULL, mr.placement ASC, mr.team_id`, [matchId]); return rows; }
export async function findResult(matchId, teamId) { const [rows] = await pool.query('SELECT id, match_id, team_id, points, kills, placement, kill_points, position_points, result_text, media_path FROM match_results WHERE match_id = ? AND team_id = ?', [matchId, teamId]); return rows[0] || null; }
export async function insertResult(input) { const [result] = await pool.query('INSERT INTO match_results (match_id, team_id, points, kills, placement, kill_points, position_points, result_text, media_path, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [input.matchId, input.teamId, input.points, input.kills, input.placement || null, input.killPoints || null, input.positionPoints || null, input.resultText || null, input.mediaPath || null, input.uploadedBy]); return result.insertId; }
export async function updateResult(input) { await pool.query('UPDATE match_results SET points = ?, kills = ?, placement = ?, kill_points = ?, position_points = ?, result_text = COALESCE(?, result_text), media_path = COALESCE(?, media_path), uploaded_by = ? WHERE match_id = ? AND team_id = ?', [input.points, input.kills, input.placement || null, input.killPoints || null, input.positionPoints || null, input.resultText || null, input.mediaPath || null, input.uploadedBy, input.matchId, input.teamId]); }
...
export async function listMatchLeaderboard(matchId) { const [rows] = await pool.query('SELECT le.match_id, le.team_id, t.name AS team_name, le.points, le.kills, le.rank, mr.placement FROM leaderboard_entries le JOIN teams t ON t.id = le.team_id LEFT JOIN match_results mr ON mr.match_id = le.match_id AND mr.team_id = le.team_id WHERE le.match_id = ? ORDER BY le.rank', [matchId]); return rows; }
export async function listGroupLeaderboard(groupId) { const [rows] = await pool.query('SELECT m.group_id, le.team_id, t.name AS team_name, SUM(le.points) AS points, SUM(le.kills) AS kills, MIN(mr.placement) AS placement, COUNT(DISTINCT m.id) AS matches_played FROM matches m JOIN leaderboard_entries le ON le.match_id = m.id JOIN teams t ON t.id = le.team_id LEFT JOIN match_results mr ON mr.match_id = m.id AND mr.team_id = le.team_id WHERE m.group_id = ? GROUP BY m.group_id, le.team_id, t.name ORDER BY points DESC, kills DESC, team_id', [groupId]); return rows; }
export async function listRoundLeaderboard(roundId) { const [rows] = await pool.query('SELECT r.id AS round_id, le.team_id, t.name AS team_name, SUM(le.points) AS points, SUM(le.kills) AS kills, MIN(mr.placement) AS placement, COUNT(DISTINCT m.id) AS matches_played FROM rounds r JOIN `groups` g ON g.round_id = r.id JOIN matches m ON m.group_id = g.id JOIN leaderboard_entries le ON le.match_id = m.id JOIN teams t ON t.id = le.team_id LEFT JOIN match_results mr ON mr.match_id = m.id AND mr.team_id = le.team_id WHERE r.id = ? GROUP BY r.id, le.team_id, t.name ORDER BY points DESC, kills DESC, team_id', [roundId]); return rows; }
