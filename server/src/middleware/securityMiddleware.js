...
export function createRateLimiter({ windowMs, max, message = 'Too many requests. Please try again later.' }) {
  ...
  return (req, res, next) => {
    ...
    current.count += 1;
    if (current.count > max) { res.setHeader('Retry-After', Math.ceil((windowMs - (now - current.startedAt)) / 1000)); return res.status(429).json({ error: { code: 'RATE_LIMITED', message } }); }
    return next();
  };
}
